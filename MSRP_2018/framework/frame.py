        ...
        try:
            ...
            for line in lines:
                ...
                for x in re.sub('[^a-zA-Z ]', '', info[4]).split():
                    # if not self.stop_list.__contains__(x):
                        sen2.append(x)
                senc1, senc2 = remove_commonword(sen1, sen2)
                sen_pair.append(senc1)
                sen_pair.append(senc2)
            ...
            for line in lines:
                ...
                for x in re.sub('[^a-zA-Z ]', '', info[4]).split():
                    # if not self.stop_list.__contains__(x):
                        sen2.append(x)
                senc1,senc2 = remove_commonword(sen1,sen2)
                # if len(senc1)>0 and len(senc2)>0:
                sen_pair.append(senc1)
                sen_pair.append(senc2)
                # if len(sen_pair)>=2:
...
def run_cnn(filter_size=2, top_k=5, full_cell=1024, iteration_num=1000):
    ...
    ## 全连接层2 ##
    # W_fc2 = weight_variable([full_cell, full_cell])
    # b_fc2 = bias_variable([full_cell])
    # # [n_samples, 7, 7, 64] ->> [n_samples, 7*7*64]
    # h_fc2 = tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

    ## 全连接层3 ———softmax层##
    W_fc3 = weight_variable([full_cell, 2])
    b_fc3 = bias_variable([2])
    prediction = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc3) + b_fc3)
    # print('prediction', prediction)

    # 损失函数  reduction_indices[]表示数的处理维度，若为0表示降到0维，即一个数
    ...
    # tf.add_to_collection("losses", tf.contrib.layers.l2_regularizer(1e-5)(W_fc3))
    tf.add_to_collection("losses", cross_entropy)
    # #
    loss = tf.add_n(tf.get_collection("losses"))
    # 训练目标
    # 学习速率为1e-4
    # train_step = tf.train.ProximalGradientDescentOptimizer(1e-4).minimize(loss)
    # train_step = tf.train.FtrlOptimizer(1e-4).minimize(loss)
    ...
    data.read_word2vec_model(r'..\data\wiki_msrp100.vec')
    ...
    for i in range(iteration_num):
        ...
        if i % 50 == 0:
            total_cross_entropy = sess.run(loss, feed_dict={xs_: batch_xs, ys: batch_ys, keep_prob: 1})
            test_loss = sess.run(loss, feed_dict={xs_: test_x, ys: test_y, keep_prob: 1})
            # print('total_cross_entropy test_loss', i, total_cross_entropy, test_loss)
            print(i, total_cross_entropy, test_loss)
        sess.run(train_step, feed_dict={xs_: batch_xs, ys: batch_ys, keep_prob: 1})  # 训练参数

    pre = sess.run(prediction, feed_dict={xs_: test_x, keep_prob: 1})
...
# print('ProximalGradientDescentOptimizer')
run_cnn(filter_size=5, top_k=10, full_cell=1024, iteration_num=50000)
print('kep,cross_entropy,clip,adam,noL2,learn_rate_1e-7 为什么还是学偏了啊！！！')
